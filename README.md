...
`v0.5.2`は、Linux x86-64、Linux ARM64、macOS Intel、macOS Apple Silicon、Windows x86-64向けのネイティブパッケージを提供する。
...
`main`は[`v0.5.2`リリースノート](docs/releases/v0.5.2.md)に記載した`0.5.2`契約を実装している。
正式版は、[`v0.5.2` GitHub Release](https://github.com/TamaT-LLC/depgraph-cli/releases/tag/v0.5.2)と公開後証跡が一致するときに限り有効である。
...
完全な互換性タプル、ストア移行、ロールバック、既知の制約は[`v0.5.2`リリースノート](docs/releases/v0.5.2.md)を参照する。

## プロジェクトの状況と公開コラボレーション

サポート対象は、検証済みの`v0.5.2`リリースを条件として確定する。
`v0.5.2`は、公式リリースと公開後証跡の公開後に現在の安定版となる。
それまでは`v0.5.1`が安定版であり、リリース候補は評価用の過去の配布物である。
