...
`main`は[`v0.5.1`リリースノート](docs/releases/v0.5.1.md)に記載した`0.5.1`契約を実装している。
...
完全な互換性タプル、ストア移行、ロールバック、既知の制約は[`v0.5.1`リリースノート](docs/releases/v0.5.1.md)を参照する。

## プロジェクトの状況と公開コラボレーション

サポート対象は、検証済みの`v0.5.1`リリースを条件として確定する。
`v0.5.1`は、公式リリースと公開後証跡の公開後に現在の安定版となる。
それまでは`v0.5.0`が安定版であり、リリース候補は評価用の過去の配布物である。
