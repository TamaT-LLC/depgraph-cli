...
safe scan は、ADR-006 に従い、depgraph に同梱された checksum 検証済み TypeScript のみを解析器として使用する。現在の固定 version は `7.0.2` である。project-local TypeScript は `package.json` と lockfile から version metadata を検出するだけで、module、native compiler、標準 library を load / execute しない。Issue #39ではbundled lexical APIに加え、隔離されたnative compiler processへinventory済みsource、許可したstatic JSON/JSONCから正規化した`paths`、worker生成のneutral config、bundled stdlibだけを公開し、Program / TypeChecker smoke queryとbounded semantic diagnosticを実行するscaffoldを追加した。TypeScript 7の規則に合わせ、`paths`は宣言元config基準、子宣言は親option全体を置換し、廃止された`baseUrl`は適用しない。Issue #41ではrepository-owned declarationだけをcanonical `symbol` / `type` nodeとsite-less `declares` / `extends` / `implements` / `instantiates` relationへ昇格し、全delta検証後にsyntax graphへatomic unionするdefinition sliceを追加した。Issue #42では同じisolated Program / TypeCheckerからESM / CJS / type-only import、re-export、path alias、package exportsとsignature / field / annotation / generic constraintのnamed type occurrenceを収集し、`web_import` / `web_reexport` / `type_use` siteと`imports` / `reexports` / `type_uses` edgeを追加した。全occurrenceを`resolved` / `candidates` / `external` / `unresolved`へ分類し、既存source graphを残したままvalidated deltaをatomic unionする。call / framework semantic siteはまだ昇格せず、`semantic-complete`も付与しない。
...
| `typescript_analysis_mode` | `semantic-import-type-graph` | TypeCheckerのversioned definition + import / re-export / type-use sliceを出力する |
...
| `typescript_typechecker_status` | `definition-import-type-graph-emitted / definition-import-type-graph-discarded` | 検証済みdefinition / dependency deltaを出力、またはtyped late failureでdelta全体を破棄 |
...
| `typescript_semantic_graph_emission` | `definition-import-type-graph-v1` | canonical definition relationに加えて`web_import` / `web_reexport` / `type_use` siteと対応edgeをcoreが許可する。callと`semantic-complete`は拒否する |
...
| `typescript_semantic_site_count` | decimal string | semantic-primaryなimport / re-export / type-use siteの監査用件数 |
...
Milestone 2 のTypeCheckerもbundled-onlyを維持し、Issue #39で安全なproject modelとfailure / attestation contract、Issue #41でdefinition graph、Issue #42でimport / re-export / type-use graphを実装した。callおよびframework semantic graphを追加で昇格する前にも以下のmatrixを維持する。project-local compiler を opt-in で許可するには、compiler artifact identity と integrity、module/config/plugin 非実行、version compatibility、sandbox を扱う後続 ADR と security review を別途必須とする。
...
- TypeScript import / re-export / type-use resolution: 実装済み（Issue #42、2026-07-19）。call / framework semantic edgeと`semantic-complete`は未実装
