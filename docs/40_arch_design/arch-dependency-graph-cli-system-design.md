...
| `typescript_analysis_mode` | `semantic-import-type-call-graph` | TypeCheckerの累積definition + import / re-export / type-use + exact-call sliceを出力する |
...
| `typescript_semantic_graph_emission` | `definition-import-type-call-graph-v1` | canonical definition/import/type-useに加えexact `call` / `calls`をcoreが許可する。`may_call`、framework semantic edge、`semantic-complete`は拒否する |
...
Milestone 2 のTypeCheckerもbundled-onlyを維持し、Issue #39で安全なproject modelとfailure / attestation contract、Issue #41でdefinition graph、Issue #42でimport / re-export / type-use graph、Issue #43でexact direct-call graphを実装した。framework semantic graphやcandidate callを追加で昇格する前にも以下のmatrixを維持する。project-local compiler を opt-in で許可するには、compiler artifact identity と integrity、module/config/plugin 非実行、version compatibility、sandbox を扱う後続 ADR と security review を別途必須とする。
...
| semantic coverage | ESM/CJS、type-only、package exports、path alias、JSX、generics、direct function / method / constructor、external / dynamic / open call、broken / newer syntax | import/type siteを `resolved / candidates / external / unresolved`、callを`resolved / external / unresolved`へ分類し、unsupported入力を黙って省略しない。callのsingleton候補をexactへ昇格しない |
...
- TypeScript exact direct-call graphとexternal / unresolved call ledger: 実装済み（Issue #43、2026-07-19）。candidate call / framework semantic edge / `semantic-complete`は未実装
