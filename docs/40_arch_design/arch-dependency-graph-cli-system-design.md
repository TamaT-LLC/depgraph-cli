...
> `v0.4.0-rc.6`までで`v0.4.0` stableは未公開である事実をrelease contractへ反映した。
> MCPを含むmainを`0.5.0`へ同期し、protocol `1.0`、Store schema `17`、
> operation journal schema `5`、`depgraph-mcp-tools-v1`、
> `depgraph-operation-v1`をv0.5 compatibility tupleへ固定する。
> 公式rc.6 schema-13 fixtureをchecksum固定してschema 17へのmigrationと
> byte不変rollback copyを検証する。Issue #359のGA gateはv0.5 stableを
...
> 2026-09-13: 次のstable minorを`v0.6.0`とし、current `main`のStore schema `19`、
> `depgraph-health-finding-v1`、CLI／MCP code-health APIを公開契約へ移す方針をADR-011へ固定した。
> 公開済み`v0.5.4`のtag、source、Store schema `17`、assetはimmutable historyとして維持する。
...
次のstable minor `v0.6.0`は、current `main`のStore schema `19`、health provenance、
`depgraph-health-finding-v1`、CLI／MCP code-health APIを含む。`v0.5.4`のpatchとして
公開せず、`refs/heads/release/0.6`とsigned tagを同じreview済みsourceへ結び付ける。
compatibility、tag、migration、maintenance contractは[ADR-011](adr-v0.6-release-contract.md)、
[v0.6.0 release note](../releases/v0.6.0.md)、[v0.5 release ADR](adr-v0.5-release-contract.md)、
[v0.5.4 release note](../releases/v0.5.4.md)をcanonicalとする。
...
evidence gateも同じreportで維持する。10,000-file reportは手動フルCIと
tag releaseでartifact化する。pull requestとmain pushではbenchmarkを実行しない。
release asset検証jobはtag workflowが生成したreportのschema、commit、
全必須metric、conservation、総合passを再検証してから公開へ進む。
fixture件数やdependency site / coverageを性能のために緩和しない。
...
- 2026-09-03: unused-code以外のfindingを`probable`上限に統一した。
  ...
  空のpolicyは設定検証後にsnapshot JSONのbounded前処理を短絡し、配布パッケージのNext.js fixtureがservice resource limitへ達する回帰を解消した。
