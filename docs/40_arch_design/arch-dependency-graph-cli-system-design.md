...
> byte不変rollback copyを検証し、v0.5 stableはexact GA baseline確定まで
> `candidate-unpinned`としてsource guardとrelease gateの双方で拒否する。
