...
新しいevent burstは進行中scanの共有cancellation tokenを発火し、全workerがprocess group / Windows Job Objectを終了してreader taskまでreapした後に次batchを開始する。cancelled attemptはsnapshotを生成・promoteしない。daemon stopも同じcleanup完了をawaitし、status APIはactive、last completed / failed / cancelled、watcher error、startup recoveryを返す。起動時に同repositoryのstaging scan / build attemptを`cancelled`へ回復するがcurrent completed pointerは変更しない。workerがrepository-complete protocolだけを出力する現段階では、planner scopeをstatusへ保持しつつatomic full replacementをsafe fallbackとし、scoped replacement worker emissionへの移行時もdaemon contractを変えない。
