...
1.185秒/1.105秒であった。bundled workerは現在repository-complete
protocolだけをadvertiseするため、incremental executorは安全にfull atomic
replacementへfallbackし、2秒の
...
- 2026-07-24: Issue #134としてwatcher / daemonをcapability-aware incremental executorへ接続。`incremental-plan-v1`のnormalized changeとanalysis closure、exact base snapshot / graph digest、canonical base graphから`worker-delta-request-v1`を生成し、neutral temporary request file、既存timeout / cancel / process-tree supervisor、bounded delta parserを通す。exact `worker-delta-v1` handshake時だけfresh incremental stagingへdurable stage / transactional applyし、通常のgraph・strict・architecture policy・cancellation gate後にpromotionする。worker failure / cancelでは旧current snapshotを維持してdaemon retry / re-coalesceへ戻し、legacy worker、workspace replan、複数adapter、planner failureはatomic full scanへfallbackする。1-file closure外node不変、failure / cancel非promotion、legacy fallbackをcore testで固定した。
