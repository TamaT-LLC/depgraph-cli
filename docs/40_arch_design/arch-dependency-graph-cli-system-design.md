...
| `typescript_analysis_mode` | `semantic-definition-graph` | TypeCheckerのversioned definition sliceだけを出力する |
...
| `typescript_typechecker_status` | `definition-graph-emitted / definition-graph-discarded` | 検証済みdefinition deltaを出力、またはtyped late failureでdelta全体を破棄 |
...
| `typescript_semantic_graph_emission` | `definition-graph-v1` | canonical definition nodeと4種のsite-less exact relationだけをcoreが許可し、semantic siteと`semantic-complete`は拒否する |
| `typescript_semantic_node_count` / `typescript_semantic_relation_count` / `typescript_semantic_issue_count` | decimal string | definition deltaとbounded issueの監査用件数 |
...
Milestone 2 のTypeCheckerもbundled-onlyを維持し、Issue #39で安全なproject modelとfailure / attestation contract、Issue #41でdefinition-only graphを実装した。type-use / import refinement / callおよびframework semantic graphを追加で昇格する前に、以下のmatrixを満たす。project-local compiler を opt-in で許可するには、compiler artifact identity と integrity、module/config/plugin 非実行、version compatibility、sandbox を扱う後続 ADR と security review を別途必須とする。

後続 semantic slice の導入前に、少なくとも次の matrix を release fixture と CI で検証する。
...
- TypeScript canonical symbol / type definition graph: 実装済み（Issue #41、2026-07-19）。type-use / call / framework semantic edgeと`semantic-complete`は未実装
