...
writer APIはheader、key schema、graph-level JSON record、node、edgeを順に直接`Write`へ流し、JSON arrayもrecord単位、XML textもbounded buffer単位で出力する。CLIの`--output`とstdoutは同じwriterを使用し、graph全体を追加の巨大文字列へ複製しない。既存JSON / DOT / Mermaidのschemaとbyte outputは変更しない。
...
- 2026-07-24: Issue #85として決定的なGraphML 1.0 exporterを実装。node / edgeのstable ID、kind、phase、profile、condition、precision、resolutionをtyped keyへ、完全なprofile / dependency site / evidenceと所有参照をcanonical JSONへ保持し、GraphML単体で再構成可能にした。XML-safeなgenerated element ID、XML 1.0特殊文字escape、Unicode保持、invalid control fail-closed、stable sort、record/text単位のbounded streaming writerを追加し、golden、round-trip、入力順非依存決定性、大容量chunk、runtime filter、CLI stdout / file E2Eで固定した。既存JSON / DOT / Mermaid出力は変更していない。
