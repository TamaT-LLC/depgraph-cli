...
| ---: | --- | --- | --- | --- |
| 1 | Lexer/parser, bounded input reader, canonical AST, and malformed corpus | 2-3 days | This ADR | Grammar accepts only one bounded statement and never echoes hostile literals |
| 2 | Closed type checker, field registry, canonical AST digest, and golden plans | 2-3 days | 1 | Node/Path/Edge/Site/Evidence bindings and operators reject every invalid combination |
| 3 | Snapshot cardinality statistics, fixed operator planner, cost admission, and explain schema | 2-3 days | 2 | Explain and execute admission are byte-identical across checkouts and SQLite row orders |
...
Parser, planner, and executor therefore remain separate follow-up Issues. The
parser cannot directly traverse, the planner cannot access raw query text, and
the executor accepts only a validated typed plan carrying exact snapshot,
graph, contract, and limit digests.
