...
- 2026-09-07: Issue #467としてsnapshot-scoped healthの読み込み・集計をStore range単位へ事前分割し（`GraphSnapshot`全体を構築しないplanner、range別budget・checkpoint・resume、cross-range参照、opt-in partial view）、`PROJ-ARC-001-ADR-009`の「Bounded range execution」節、MCP `execution`/`partial`/`allow_partial_ranges`、[resumable analysis validation](../40_arch_design/resumable-analysis-validation.md)のranged health fixture計測を追加
