...
公開済み`v0.5.4`のpackとStore schema `17`は履歴検証用であり、v0.6.1のarchiveやschema `19`と混在させない。
RC tagはworkflowのexact source SHAへ結合し、sequenceは先頭ゼロを含まない正整数に限る。
stable `v0.6.1`はremote `main`、`release/0.6`、tag、exact Full CIが一致するまで拒否する。
v0.6.1のrelease contractにはStore schema `19`とcode-health契約／APIを含め、通常archive、compiler pack、manifestのversionとtargetを同じsource gateで検証する。
