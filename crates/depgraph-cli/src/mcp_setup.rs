...
mod tests {
    ...
    fn artifact_layout_is_shared_by_version_and_target_but_stores_are_repository_scoped() {
        let cache = tempfile::tempdir().unwrap();
        let layout =
            ArtifactLayout::new(cache.path(), "0.5.1", "x86_64-unknown-linux-gnu").unwrap();
        let first = git_repository();
        let second = git_repository();
        assert_eq!(
            layout.target_root,
            ArtifactLayout::new(cache.path(), "0.5.1", "x86_64-unknown-linux-gnu")
                .unwrap()
                .target_root
        );
        assert_ne!(
    ...
    fn archive_paths_reject_traversal_and_cross_package_entries() {
        assert_eq!(
            safe_archive_path(
                Path::new("depgraph-0.5.1-target/bin/depgraph"),
                "depgraph-0.5.1-target"
            )
            .unwrap(),
            Path::new("depgraph-0.5.1-target/bin/depgraph")
        );
        for unsafe_path in [
            "../outside",
            "/absolute",
            "depgraph-0.5.1-target/../outside",
            "other-package/bin/depgraph",
        ] {
            assert!(
                safe_archive_path(Path::new(unsafe_path), "depgraph-0.5.1-target").is_err(),
                "{unsafe_path}"
            );
        }
    }
    ...
    fn tar_extraction_materializes_a_bounded_package_tree() {
        ...
        let package_root = "depgraph-0.5.1-target";
        {
