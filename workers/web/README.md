...
| `typescript_semantic_graph_emission` | `definition-import-type-call-graph-v2` | Canonical definitions plus import/re-export/type-use sites, exact `call`/`calls`, and closed candidate `call`/`may_call` edges are allowed; framework semantic edges and `semantic-complete` remain forbidden |
