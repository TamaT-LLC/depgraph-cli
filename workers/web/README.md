...
| --- | --- | --- | --- |
| Valid release-adjacent TypeScript 7.0.2 native compiler | Use the bundled compiler for syntax analysis plus the isolated Program / TypeChecker definition, import, re-export, type-use, exact direct-call, and closed candidate-call graph | `source=bundled`, `version=7.0.2`, `selection=bundled-only` | Not needed |
...
| `typescript_analysis_mode` | `semantic-import-type-call-graph` | Program / TypeChecker emits the cumulative definition + import/re-export/type-use + exact/candidate-call slice |
...
| `typescript_semantic_graph_emission` | `definition-import-type-call-graph-v2` | Canonical definitions plus import/re-export/type-use sites, exact `call`/`calls`, and closed candidate `call`/`may_call` edges are allowed; a detected framework additionally requires a complete framework capability ledger |
...
| `typescript_semantic_diagnostics` / `typescript_emitted_semantic_diagnostics` | decimal counts | Total and retained deterministic TypeChecker diagnostics; either nonzero count blocks `semantic-complete` |
...
| Semantic issue, compiler diagnostic, unsupported/skipped/unresolved input, late semantic-delta rejection, or incomplete detected-framework capability | Retain the syntax graph and semantic ledger where valid, but do not overstate completeness | `semantic-complete` is absent and coverage records a stable incompleteness reason |
...
The isolated Program / TypeChecker now emits the cumulative `definition-import-type-call-graph-v2` slice on every successful Web scan. Imports, re-exports, named type occurrences, and admitted non-module-loader call expressions, constructor calls, and tagged templates are classified as `resolved`, `candidates`, `external`, or `unresolved`. A call can become a candidate only when worker-owned flow analysis proves a complete, immutable local `const` function-value flow, or a `const` receiver initialized by a closed finite flow of zero-argument `new Class()` expressions (a direct expression or conditional). For the latter, every class must be non-inheriting and contain only direct methods—no decorators, constructor, fields, accessors, static blocks, or other members—and the receiver must have exactly one non-declaration use: the nonoptional direct method or tagged invocation under analysis. Every canonical target is retained in sorted order and produces a same-site `may_call` edge. A profile may claim `semantic-complete` only when syntax coverage is complete, the emitted v2 delta and worker-owned project model are ready, skipped/unsupported/unresolved/semantic-issue/compiler-diagnostic counts are all zero, and `project_code_executed=false`; candidate and external sites do not block that claim. Detected Next.js, Astro, TanStack Router, and TanStack Start profiles must additionally report every required common and framework-specific capability as complete. Semantic output does not change the `bundled-only` compiler decision; any future project-local execution mode requires a separate threat-model decision and an explicit, non-safe profile.
...
An exact `calls` edge is emitted only when the caller has a canonical execution scope, the resolved signature correlates to one repository declaration, and dispatch is statically closed: a direct function/constructor, static or private method, `super`, or a method invoked on a syntactically fresh expression. Closed candidates are deliberately separate: immutable local `const` aliases and complete conditional branches of direct local callables use `typescript-closed-local-call-flow-v1`; `typescript-closed-local-fresh-instance-flow-v1` requires a `const` receiver initialized by a closed finite flow of zero-argument `new Class()` branches (a direct expression or conditional). Each class must be a non-inheriting plain class declaration with only direct methods (no decorators, constructor, fields, accessors, static blocks, or other members), and the receiver must have exactly one non-declaration use: the direct nonoptional method or tagged invocation being analyzed. Both remain `candidates / overapprox` even with one target and emit one `may_call` per canonical target. Stdlib and workspace-external declarations use an `external_system` sentinel. Mutable aliases, parameters, fields, returns, incomplete branches, candidate-receiver constructor arguments or constructors, class fields/accessors/static blocks/decorators/non-method members, inheritance, every receiver alias/property read or write/argument/return/capture/escape/second use, overloads, unions, interface signatures, open receiver dispatch, missing declarations, broken source, and callable bodies without a canonical definition remain reason-bearing unresolved call sites targeting `unknown_target`.
...
Release archives keep the native compiler and its standard library tree under `libexec/typescript/lib`. The release manifest pins the component version, entry point, and canonical whole-tree SHA-256, so the `depgraph` core launcher rejects missing, added, symlinked, or modified files before the Web worker starts. The Web worker manifest also attests TypeScript `7.0.2`, the sorted versioned capability set, and the exact `astro-parser-wasm@4.0.0` and `typescript-native-compiler@7.0.2` runtime component identities. Its `--version` handshake must report the same TypeScript and capability unit; manifest or handshake omissions, additions, order changes, unknown fields, and version mismatches fail closed. Direct worker execution trusts its adjacent installation and is intended for protocol development; the verified safe-scan integrity contract requires launching through `depgraph`.

Astro frontmatter is parsed by the bundled Astro compiler 4.0.0; the release directory must keep the whole-tree-attested `astro/astro.wasm` component next to `worker.mjs`. If compiler positions are unavailable, the worker falls back to a source tokenizer and emits heuristic evidence plus a diagnostic.
...
TypeChecker semantic diagnostics and the definition/import/re-export/type-use/exact/candidate-call graph run inside the isolated compiler boundary. Candidate inference is limited to the audited closed local flows above; plugin-defined virtual modules, code-based route construction, and all other dynamic call flows remain unresolved or diagnostic-only in safe mode. A late validation failure discards the complete semantic delta, including its sites, edges, generated module-initializer symbols, and semantic-only sentinels, while retaining the source dependency graph.
