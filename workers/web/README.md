...
| `typescript_semantic_graph_emission` | `definition-import-type-call-graph-v2` | Canonical definitions plus import/re-export/type-use sites, exact `call`/`calls`, and closed candidate `call`/`may_call` edges are allowed; only a pure non-framework profile may claim `semantic-complete` |
...
| Semantic issue, compiler diagnostic, unsupported/skipped/unresolved input, late semantic-delta rejection, or detected framework | Retain the syntax graph and semantic ledger where valid, but do not overstate completeness | `semantic-complete` is absent and coverage records a stable incompleteness reason |
