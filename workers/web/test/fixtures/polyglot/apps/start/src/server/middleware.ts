...
export const rootAuditMiddleware = createMiddleware({ type: "request" }).server(async ({ next }) => next());
export const authMiddleware = createMiddleware({ type: "function" }).server(async ({ next }) => next());
export const pathlessAuditMiddleware = createMiddleware({ type: "function" }).server(async ({ next }) => next());
